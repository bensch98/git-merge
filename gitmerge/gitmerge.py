...
from gitmerge.errors import MissingArgumentException
...
@click.option('--src', '-s', required=False, help='Path to source repo which the contributions were committed.')
@click.option('--srcs', required=False, help='Path to a directory, where all subfolders with a .git directory are checked.')
...
def merge(author, src, srcs, dest, company, _list, since):
  
  # check if arguments are missing
  if src is None and srcs is None:
    raise MissingArgumentException(["--src", "--srcs"])
